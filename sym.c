...
static int loadSym(struct symtable *sym, char *name,
	...
	if (fread(sym, sizeof(struct symtable), 1, Symfile) != 1)
		return (-1);
	...
	if (loadit == 0) {
		...
		if (name != NULL && !strcmp(name, SymText)) {
			...
			if (stype >= S_STRUCT && stype == sym->stype)
				loadit = 1;
		}
	}
	...
	if (loadit) {
		...
		if (sym->name == NULL)
			fatal("Unable to malloc name in loadSym()");
...
#ifdef DEBUG
	if (id != 0)
		fprintf(stderr, "findlocl() searching for id %d\n", id);
	if (name != NULL)
		fprintf(stderr, "findlocl() searching for name %s\n", name);
#endif
