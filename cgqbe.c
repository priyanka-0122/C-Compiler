...
int cgloadvar(struct symtable *sym, int op) {
	...
	if (op == A_PREINC || op == A_PREDEC) {
		if (sym->st_hasaddr || qbeprefix == '$') {
			...
			fprintf(Outfile, "\t%c%s =%c add %c%s, %d\n", qbeprefix, sym->name, cgprimtype(sym->type), qbeprefix, sym->name, offset);
	}
...
// Divide the first temporary by the second and return the number of the temporary with the result
...
int cgcall(struct symtable *sym, int numargs, int *arglist, int *typelist) {
	...
	if (sym->type == P_VOID)
		...
		fprintf(Outfile, "\t%%.t%d =%c call $%s(", outr, cgprimtype(sym->type), sym->name);
...
int cgcompare_and_jump(int ASTop, int parentASTop, int r1, int r2, int label, int type) {
...
// Dereference a pointer to get the value it points at into a new temporary
